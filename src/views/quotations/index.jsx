...
export default function Quotations() {
  ...
  const load = () => {
    setLoading(true);
    api
      .listQuotations(statusFilter === 'ALL' ? undefined : statusFilter)
      .then(setQuotations)
      ...
      <Stack direction={{ xs: 'column', sm: 'row' }} sx={{ alignItems: { sm: 'center' }, justifyContent: 'space-between', gap: 2 }}>
        <Box>
          ...
          <Typography color="text.secondary" sx={{ mt: 0.5 }}>
            Genera y da seguimiento a las cotizaciones para tus clientes.
          </Typography>
        </Box>
        <Button component={Link} to="/quotations/new" variant="contained" startIcon={<AddRoundedIcon />}>
          Nueva cotización
        </Button>
      </Stack>
              ...
              {loading ? (
                ...
                quotations.map((quotation) => (
                  <TableRow hover key={quotation.folio}>
                    <TableCell>
                      <Button component={Link} to={`/quotations/${quotation.folio}`} sx={{ p: 0, minWidth: 0 }}>
                        {quotation.folio}
                      </Button>
                    </TableCell>
                    <TableCell>
                      <Typography fontWeight={600}>{quotation.customer.name}</Typography>
