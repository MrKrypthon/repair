...
export default function QuotationDetail() {
  ...
  const load = () =>
    api
      .getQuotation(folio)
      .then(setQuotation)
      .catch(() => setMessage({ type: 'error', text: 'No se pudo cargar la cotización.' }))
  ...
  const changeStatus = (status) => {
    ...
    api
      .updateQuotationStatus(folio, status)
      .then(setQuotation)
      .catch(() => setMessage({ type: 'error', text: 'No se pudo actualizar el estado de la cotización.' }))
  ...
  const convert = () => {
    ...
    setMessage({ type: '', text: '' });
    api
      .convertQuotation(folio)
      .then((order) => navigate(`/service-orders/${order.folio}`))
      .catch(() => {
        setMessage({ type: 'error', text: 'No se pudo convertir la cotización en orden.' });
        setSaving(false);
      });
  };
      ...
      <Stack direction={{ xs: 'column', sm: 'row' }} sx={{ alignItems: { sm: 'center' }, justifyContent: 'space-between', gap: 2 }}>
        <Box>
          <Stack direction="row" spacing={1.5} sx={{ alignItems: 'center' }}>
            ...
            <Chip
              label={statusLabels[quotation.status] || quotation.status}
        ...
        <Grid size={{ xs: 12, md: 8 }}>
          <Stack spacing={3}>
            <MainCard title="Falla reportada">
              <Typography>{quotation.issueDescription}</Typography>
            </MainCard>
                  ...
                  <TableBody>
                    {quotation.items.map((item) => (
                      <TableRow key={item.id}>
                        ...
                        <TableCell align="right">{money(item.unitPrice * item.quantity)}</TableCell>
                      </TableRow>
                    ))}
                  </TableBody>
                </Table>
          ...
          <Stack spacing={3}>
            <MainCard title="Detalles">
              <Stack spacing={1.5}>
                <Stack direction="row" sx={{ justifyContent: 'space-between' }}>
                  ...
                  <Typography>{new Date(quotation.createdAt).toLocaleDateString('es-MX')}</Typography>
                </Stack>
                <Stack direction="row" sx={{ justifyContent: 'space-between' }}>
                  <Typography color="text.secondary">Válida hasta</Typography>
                  <Typography>{quotation.validUntil ? new Date(quotation.validUntil).toLocaleDateString('es-MX') : '-'}</Typography>
                </Stack>
                <Stack direction="row" sx={{ justifyContent: 'space-between' }}>
                  <Typography color="text.secondary">Creada por</Typography>
                  <Typography>{quotation.createdBy?.name || '-'}</Typography>
                </Stack>
                {quotation.serviceOrder && (
                  <Stack direction="row" sx={{ justifyContent: 'space-between' }}>
                    <Typography color="text.secondary">Orden generada</Typography>
                    <Button component={Link} to={`/service-orders/${quotation.serviceOrder.folio}`} sx={{ p: 0, minWidth: 0 }}>
                      {quotation.serviceOrder.folio}
                    </Button>
                  </Stack>
                )}
              </Stack>
            ...
            <MainCard title="Acciones">
              <Stack spacing={1.5}>
                ...
                {quotation.status === 'APPROVED' && !quotation.serviceOrder && (
                  <Button fullWidth variant="contained" startIcon={<SyncAltRoundedIcon />} disabled={saving} onClick={convert}>
                    Convertir a orden de servicio
                  </Button>
                )}
                {['REJECTED', 'CONVERTED'].includes(quotation.status) && (
                  <Typography color="text.secondary" variant="body2">
                    Esta cotización ya no admite más acciones.
                  </Typography>
                )}
              </Stack>
            </MainCard>
