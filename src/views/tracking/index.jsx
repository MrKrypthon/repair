...
export default function Tracking() {
  ...
  const decideBudget = (budgetStatus) => {
    setSavingBudget(true);
    api
      .publicBudget(token, budgetStatus)
      .then(() => setOrder((current) => ({ ...current, budgetStatus })))
      ...
      <Card sx={{ maxWidth: 520, mx: 'auto', mt: 8 }}>
        <CardContent>
          ...
          <Typography color="text.secondary" sx={{ mt: 1 }}>
            Verifica el código de seguimiento con el taller.
          </Typography>
        </CardContent>
      </Card>
    );
  ...
  return (
    <Card sx={{ maxWidth: 720, mx: 'auto', mt: { xs: 3, md: 8 } }}>
      <CardContent sx={{ p: { xs: 3, md: 5 } }}>
        <Stack spacing={3.5}>
          <Box>
            <Typography color="primary" fontWeight={700} sx={{ letterSpacing: 1 }}>
              FIXTRACK
            </Typography>
            ...
            <Typography color="text.secondary" sx={{ mt: 0.5 }}>
              Orden {order.folio} · {order.device.brand} {order.device.model}
            </Typography>
          </Box>

          {isTerminalOutcome ? (
            <Alert severity={order.status === 'CANCELADO' ? 'error' : 'warning'} icon={<CancelRoundedIcon />}>
              {order.status === 'CANCELADO' ? 'Esta orden fue cancelada.' : 'El equipo no tuvo reparación.'} Contacta al taller si tienes
              dudas.
            </Alert>
          ) : (
              ...
              <Box>
                ...
                {order.estimatedDeliveryAt && (
                  <Typography color="text.secondary">
                    Entrega estimada: {new Date(order.estimatedDeliveryAt).toLocaleDateString('es-MX')}
                  </Typography>
                )}
              </Box>
            </Stack>
          ...
          {order.estimatedCost && (
            <Box sx={{ p: 2.5, border: '1px solid', borderColor: 'divider', borderRadius: 2 }}>
              ...
              <Typography variant="h2" sx={{ mt: 1 }}>
                ${Number(order.estimatedCost).toLocaleString('es-MX', { minimumFractionDigits: 2 })}
              </Typography>
              {order.budgetStatus === 'PENDING' ? (
                <Stack direction="row" spacing={1} sx={{ mt: 2 }}>
                  <Button fullWidth variant="contained" onClick={() => decideBudget('APPROVED')} disabled={savingBudget}>
                    Autorizar
                  </Button>
                  <Button fullWidth variant="outlined" color="error" onClick={() => decideBudget('REJECTED')} disabled={savingBudget}>
                    Rechazar
                  </Button>
                </Stack>
              ) : (
                <Typography color={order.budgetStatus === 'APPROVED' ? 'success.main' : 'error.main'} sx={{ mt: 1 }}>
                  {order.budgetStatus === 'APPROVED' ? 'Presupuesto autorizado' : 'Presupuesto rechazado'}
                </Typography>
              )}
            </Box>
          )}
                ...
                <Stack direction="row" spacing={1.5} key={`${event.createdAt}-${event.newStatus}`}>
                  ...
                  <Box>
                    ...
                    <Typography variant="body2" color="text.secondary">
                      {event.note || 'Actualización de reparación'} · {new Date(event.createdAt).toLocaleString('es-MX')}
                    </Typography>
                  </Box>
                </Stack>
              ))}
          ...
          <Divider />
          <Typography variant="body2" color="text.secondary">
            Esta página se actualiza sola. Para más información, contacta directamente con el taller.
          </Typography>
        </Stack>
      </CardContent>
