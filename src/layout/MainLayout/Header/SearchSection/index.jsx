          ...
          <ul style={{ padding: 0 }}>
            ...
            {results.serviceOrders.map((order) => (
              <ListItemButton key={order.folio} onClick={() => onNavigate(`/service-orders/${order.folio}`)}>
                <ListItemText
                  primary={`${order.folio} · ${order.device.brand} ${order.device.model}`}
                  secondary={`${order.customer.name} · ${statusLabel(order.status)}`}
                />
              </ListItemButton>
            ))}
          </ul>
...
export default function SearchSection() {
  ...
  const dropdown = showDropdown && (
    <Popper
      ...
      sx={{ zIndex: 1200, width: anchorRef.current?.offsetWidth || 320 }}
